  	...
  	if (e.queryString.localeCompare("")==0) {
        ...
        return template0.evaluate().setTitle("Eina de creació de presentacions")
          ...
          var template0 = HtmlService.createTemplateFromFile('pantalla-info');
              ...
              template0.id = "https://script.google.com/macros/s/AKfycbyaz-KrhOESpoUOrGHncEnSSWixoxXJ6Zvh4Xm7tgO32Az4TIpd/exec";               
              template0.config = "Config";     
          return template0.evaluate().setTitle("Error")
			...
			var cache = CacheService.getPublicCache();
			//cache.put("id", e.parameter.ID,30000);  
			cache.put("id", id,30000);        
			cache.put("Pestanya_configuracio",e.parameter.config,30000);  
			cache.put("config",e.parameter.config,30000);    
			...
			var cnf_fll = spread.getSheetByName(e.parameter.config); 
			...
			for (var v = 1; v < cnf_values.length; v++) {
	          eval("var "+cnf_values[v][1]+"=\""+cnf_values[v][4]+"\"");
			}
...
//Recollida de dades de les presenatcions permeses i de les plantilles
function recollidaPlantilles(){
  ...
  for (var v = 1; v < cnf_values.length; v++) {
    ...
    if (cnf_values[v][1].indexOf("Plantilla_")>-1){
      ...
      var valVar=cnf_values[v][5].replace(new RegExp("'", 'g'),"`"); 
      eval(str+"='"+valVar+"'");
    }    
  }
...
function salvarDades(retorn){
  var permis= comprovacioPermis();
  var missatge=new Array();  
  if (permis[2]!='S'){
    var userEmail = Session.getActiveUser().getEmail();    
    missatge[0]="No s'ha efectuat cap canvi atès que l'usuari actual ("+userEmail+") no està autoritzat a realitzar canvis";  
    return missatge;    
  } else {
    var cache = CacheService.getPublicCache();
    var id = cache.get('id');
...
function crear_fitxa_apadrina(id_element_apadrinat) {
 ...
 //variables a cercar
  ...
  if (ss_url==""){
    var nomFull=nom_crp+"-"+nom_centre
